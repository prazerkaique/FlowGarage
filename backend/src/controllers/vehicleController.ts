...
export const generatePublicCatalogToken = async (req: Request, res: Response) => {
  try {
    const garageId = req.user?.garageId;
...
export const searchVehicles = async (req: Request, res: Response) => {
  try {
    ...
    if (steering) whereClause.steering = steering;
    if (fuel) whereClause.fuel = fuel;
    if (color) whereClause.color = { [Op.iLike]: `%${color}%` };

    // Filtros de faixa
    if (yearMin || yearMax) {
      whereClause.year = {};
      if (yearMin) whereClause.year[Op.gte] = yearMin;
      if (yearMax) whereClause.year[Op.lte] = yearMax;
    }
    ...
    if (mileageMin || mileageMax) {
      whereClause.mileage = {};
      if (mileageMin) whereClause.mileage[Op.gte] = mileageMin;
      if (mileageMax) whereClause.mileage[Op.lte] = mileageMax;
    }

    // Filtros booleanos
    if (leatherSeats) whereClause.leatherSeats = leatherSeats === 'true';
    if (multimediaSystem) whereClause.multimediaSystem = multimediaSystem === 'true';
    if (absBreaks) whereClause.absBreaks = absBreaks === 'true';
    if (xenonHeadlights) whereClause.xenonHeadlights = xenonHeadlights === 'true';
    if (electricLock) whereClause.electricLock = electricLock === 'true';
    if (armored) whereClause.armored = armored === 'true';
    if (auction) whereClause.auction = auction === 'true';
